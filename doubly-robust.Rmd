...
This tutorial primarily deals with construction of the estimator itself, under conditions where we assume that we can approximate either the outcome or the propensity score well with linear models. 
We can relax assumptions about the complexity of the underlying functionals by incorporating cross-fitting as well, as described in [[Chernozhukov et al. 2018]{.underline}]https://arxiv.org/abs/1608.00060). This allows us to use machine learning methods for both the outcome and propensity score models, and can be particularly useful when the true relationship between the covariates and the outcome or treatment assignment is complex. 

You can think of doubly robust estimation as statistics’ version of the [[Swiss cheese model]{.underline}](https://www.nytimes.com/2020/12/05/health/coronavirus-swiss-cheese-infection-mackay.html). Each modeling strategy – propensity score estimation and outcome regression modeling – is a layer of defense against bias. Neither may be perfect, but by layering the strategies on top of each other, we give ourselves more opportunities to block bias from degrading our estimates.
