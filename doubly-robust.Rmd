...
This tutorial explores a doubly robust estimator that works if we can effectively approximate the outcome and/or the propensity score with linear models. We can relax our assumptions about the complexity of the underlying functionals by incorporating cross-fitting, as described in [[Chernozhukov et al. 2018]{.underline}](https://arxiv.org/abs/1608.00060). Cross-fitting allows us to use machine learning methods to fit the outcome and propensity score models, which can be particularly useful if the true relationship between the covariates and the outcome or treatment assignment is complex. 

You can think of doubly robust estimation as statistics’ version of the [[Swiss cheese model]{.underline}](https://www.nytimes.com/2020/12/05/health/coronavirus-swiss-cheese-infection-mackay.html). Each modeling strategy – propensity score estimation and outcome regression modeling – is a layer of defense against bias. Neither perfectly guards against bias, but by layering the strategies on top of each other, we give ourselves more opportunities to block bias from degrading our estimates.
...
[^proof]: The following proof is excerpted with minimal adaptation from Funk et al. [[Web Appendix 1]{.underline}](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3070495/bin/supp_173_7_761__index.html), which is in turn adapted from [[Tsiatis 2006]{.underline}](https://link.springer.com/book/10.1007/0-387-37345-4).
