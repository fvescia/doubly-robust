...
This tutorial explores a doubly robust estimator that works if we can effectively approximate the true outcomes and/or propensity scores with linear models. We can relax our assumptions about the complexity of the underlying functionals [^func] by incorporating cross-fitting, as described in [[Chernozhukov et al. 2018]{.underline}](https://arxiv.org/abs/1608.00060). Cross-fitting allows us to use machine learning methods to fit the outcome and propensity score models, which can be particularly useful if the true relationship between the covariates and the outcome or treatment assignment is complex. 
