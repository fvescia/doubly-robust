...
Let's break down the doubly robust estimator mathematically and programmatically. We'll use the notation defined above for our proofs, and we'll fit our models on simulated data. If you'd like to see how the data were generated, you can find that code on [[GitHub]{.underline}](https://github.com/fvescia/doubly-robust). The main things to know are that treatment ($D$) is a function of all three covariates ($\bf{Z}$) in the data, while outcomes ($Y$) are a function of two of the covariates ($Z_1$ and $Z_3$). Outcomes are *not* a function of treatment, so the true treatment effect is zero.
...
Recall that doubly robust estimation combines two modeling strategies, outcome regression modeling and propensity score estimation. Let's start with our outcomes: we model the relationships between characteristics ($\bf{Z}$) and outcomes ($Y$) in each exposure group, then use those models to predict outcomes ($\hat{Y}$) for everyone. ($Z_2$) This means each individual has one *observed* outcome, for the condition they are assigned to, and two *predicted* outcomes, one each for treatment and one for control.
...
ps <- glm(d ~ z1 + z3, data = data, family = 'binomial') # Logit

# Predict propensity scores
data <- data %>% mutate(ps = predict(ps, data))
```
...
if (s == 1){
  ...
  # Fit propensity score model
  ps <- glm(d ~ z1 + z3, data = data, family = 'binomial')
}
...
if (s == 2){
  ...
  # Fit propensity score model
  # Note z3 omitted
  ps <- glm(d ~ z1, data = data, family = 'binomial')
}
...
data <- data %>% mutate(dr1 = 
                        ...
                        dr1 =
                          ifelse(d == 1,
                                 (yhat0),
                                 ((y / (1 - ps) - ((yhat0 * ps) / (1 - ps))))))

# Compute ATE
return (mean(data$dr1) - mean(data$dr1))
}
```

```{r}
# "Starter row" to create table
df <- tibble(scenario = 99, sample_size = 99, bias = 99)

# Populate table

# 3 scenarios
for (s in 1:3){
  # 4 sample sizes per scenario
  for (n in c(100, 500, 1000, 2000)){
    ...
    # No true effect per DGP; any effect is bias
    bias = mean(ates)
    df = df %>% add_row(scenario = s, sample_size = n, bias = bias)
  }
}
...
# Set up
avgs <- c()
tab <- tibble(n = 99, avg = 99)

# Simulate 10 cohorts of size n
for (n in c(10, 20, 30)){
  for (i in 1:10){
  df <- tibble(x = 1:10, y = rnorm(10, mean = 0, sd = 1))
  # Bootstrap 10 complete resamples per cohort
    for (i in 1:10){
      resamp <- df %>% slice_sample(prop=1, replace=T)
      avgs <- append(avgs, mean(resamp$y))
    }
  }
  avg <- mean(avgs)
  tab <- tab %>% add_row(n = n, avg = avg)
}
